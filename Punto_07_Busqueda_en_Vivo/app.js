...
function pintarFila() {
    ...
    /*===========================================================*/
    /* PARTE AGREGADA DEL PUNTO 7

    /* Con reviso si el arreglo turnos esta vacio*/
    if (turnos.length === 0) {

        /* Si no queda ningún turno, muestro el mensaje*/
        mensajeVacio.style.display = "block";

    }
    else {

        /* Si todavía quedan turnos, oculto el mensaje*/
    ...
    /* Con vuelvo a aplicar el filtro despues de recontruir todo los
    turnos*/
    aplicarBusqueda();
...
/* Con conecto el buscador con aplicarBusqueda y el evento input se
ejecuta cada vez que se escribe o se borra una letra*/
...
/*===========================================================*/
/* AQUI COMIENZA EL PUNTO 7 DEL TALLER
/*===========================================================*/

/* Con creo la funcion function aplicarBusqueda() {} aplicarBusqueda 
para filtrar los turnos mientras la persona escribe en el campo de 
busqueda*/
function aplicarBusqueda() {

    /* Con const textoBuscado = buscador.value; leo el texto escrito en 
    el buscador*/
    const textoBuscado = buscador.value;

    /* Con const textoBuscadoMinusculas = textoBuscado.toLowerCase(); 
    convierto el texto a minusculas para que la busqueda no dependa 
    de mayusculas o minusculas*/
    const textoBuscadoMinusculas = textoBuscado.toLowerCase();

    /* Con const elementosTurno = document.querySelectorAll(".turno"); 
    busco todos los elementos (li) que representan turnos*/
    const elementosTurno = document.querySelectorAll(".turno");

    /* Con for (const elementoTurno of elementosTurno) {} 
    recorro cada turno que esta pintado en la pantalla*/
    for (const elementoTurno of elementosTurno) {

        /* Con const contenidoTurno = elementoTurno.textContent.toLowerCase();
        leo todo que contiene el turno y tambien lo convierto a minuscula*/
        const contenidoTurno = elementoTurno.textContent.toLowerCase();

        /* Con const coincide = contenidoTurno.includes(textoBuscadoMinusculas);
        compruebo si el contenido del turno incluye el texto escrito por 
        la persona*/
        const coincide = contenidoTurno.includes(textoBuscadoMinusculas);

        /* Con if (coincide === true) {} si el contenido coincide con la 
        busqueda, se retira la clase que oculta el turno*/
        if (coincide === true) {

            elementoTurno.classList.remove("turno--oculto");

            /* Con else {} si el contenido no coincide, se agrega la clase
            turno--oculto para ocultarlo*/

        }
        else {

            elementoTurno.classList.add("turno--oculto");
        }
